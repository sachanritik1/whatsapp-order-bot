...
const heuristicIntent = (message: string, session: OrderSession | null): Intent => {
  ...
  if (isGenericPriceQuestion(message) || containsAny(normalized, ["price", "cost", "how much", "rate", "mrp"])) {
...
const buildHeuristicPlan = (input: PlanTurnInput): TurnPlan => {
  ...
  const extraction = {
    ...
    cityOrPincode: extractCityOrPincode(input.userMessage, activeSession) ?? activeSession?.cityOrPincode ?? null
  };
  ...
  if (intent === "price_question") {
    ...
    if (!input.productSearch && (explicitProductQuery || activeSession?.product)) {
      return decodeTurnPlan({
        ...
        requestedTools: [{ name: "searchProducts", reason: "Need product match to answer price question." }],
  ...
  if (intent === "new_order") {
    ...
    const catalogReply =
      missingForFreshOrder.length === 0
        ...
        : input.productSearch && input.productSearch.query !== "catalog" && input.productSearch.matches.length === 0
  ...
  if (intent === "clarify") {
    return decodeTurnPlan({
      ...
      selectedProductId,
      requestedTools: [],
      replyText: "I can help with products, prices, shipping, or a new order. What would you like to do?",
      confidence: 0.7,
      provider: "heuristic"
    });
  }
...
const normalizeAlternateTurnPlan = (
  ...
  const firstMessage =
    Array.isArray(parsed.messages) && typeof parsed.messages[0] === "object" && parsed.messages[0] !== null
      ? (parsed.messages[0] as { readonly content?: unknown })
      : null;
  const firstAction =
    Array.isArray(parsed.actions) && typeof parsed.actions[0] === "object" && parsed.actions[0] !== null
      ? (parsed.actions[0] as {
  ...
  const orderDetails =
    firstAction && typeof firstAction.orderDetails === "object" && firstAction.orderDetails !== null
    ...
    return LLMService.of({
      planTurn: (input) =>
        hasOpenRouter
              ...
              try: async () => {
                ...
                try {
                  ...
                  try {
                    ...
                    const normalized = normalizeAlternateTurnPlan(parsed, input);
                    if (normalized) {
                      logWarn("openrouter.normalized_response", {
                        note: "Normalized non-standard OpenRouter output into TurnPlan.",
                        rawContent: content
                      });
                      return normalized;
                    }
                    ...
                    throw new IntegrationError({
                      service: "openrouter",
                      message: "OpenRouter returned invalid TurnPlan JSON.",
                      cause: {
                        rawContent: content,
                  ...
                  throw new IntegrationError({
                    service: "openrouter",
                    message: "OpenRouter returned invalid JSON content.",
                    cause: {
                      rawContent: content,
                ...
                cause instanceof IntegrationError
                  ...
                  : new IntegrationError({
                      service: "openrouter",
                      message: "OpenRouter turn planning request failed.",
                      cause: serializeForLog(cause)
                    })
            }).pipe(
              Effect.catchAll((error) =>
                Effect.sync(() => {
                  logWarn("openrouter.fallback", {
                    reason: "OpenRouter turn planning failed; using heuristic planner.",
          ...
          : Effect.sync(() => buildHeuristicPlan(input))
    });
  })
);
