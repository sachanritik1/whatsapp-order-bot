...
const buildGeminiTurnPlanResponseSchema = (): JsonRecord => {
  ...
  return toGeminiCompatibleJsonSchema(jsonSchema) as JsonRecord;
};

const GEMINI_TURN_PLAN_RESPONSE_SCHEMA = buildGeminiTurnPlanResponseSchema();
...
const parseRemoteTurnPlanContent = (
  content: string,
  input: PlanTurnInput,
  provider: RemoteTurnPlanProvider
): TurnPlan => {
  ...
  for (const candidate of candidates) {
    try {
      ...
      const coerced = coerceParsedTurnPlan(parsed, input, provider);

      try {
        return decodeTurnPlan(coerced);
      } catch (schemaCause) {
        const normalized = normalizeAlternateTurnPlan(parsed, input, provider);
        if (normalized) {
        ...
        throw new IntegrationError({
          service: provider,
          message: `${provider === "gemini" ? "Gemini" : "OpenRouter"} returned invalid TurnPlan JSON.`,
          cause: {
            rawContent: content,
  ...
  throw new IntegrationError({
    service: provider,
    message: `${provider === "gemini" ? "Gemini" : "OpenRouter"} returned invalid JSON content.`,
    cause: {
      rawContent: content,
...
const parseGeminiStructuredTurnPlanContent = (
  content: string,
  input: PlanTurnInput
): TurnPlan => {
  let parsed: unknown;
  ...
  try {
    ...
    throw new IntegrationError({
      service: "gemini",
      message: "Gemini returned invalid structured JSON content.",
      cause: {
        rawContent: content,
  ...
  if (!isJsonRecord(parsed)) {
    throw new IntegrationError({
      service: "gemini",
      message: "Gemini returned a non-object structured response.",
      cause: {
        rawContent: content,
        parsed
      }
  ...
  const coerced = coerceParsedTurnPlan(parsed, input, "gemini");
  ...
  try {
    ...
    throw new IntegrationError({
      service: "gemini",
      message: "Gemini returned structured JSON that failed TurnPlan validation.",
      cause: {
        rawContent: content,
        coercedCandidate: coerced,
        parseError: serializeForLog(cause)
...
const callOpenRouterPlan = async (
  ...
  return parseRemoteTurnPlanContent(content, input, "openrouter");
};
...
const callGeminiPlan = async (
  ...
  return parseGeminiStructuredTurnPlanContent(content, input);
};
